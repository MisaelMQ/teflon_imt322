...
Motor left(16,4,36,39);
Motor right(0,2,34,35);

volatile long encoder0Pos = 0;    // encoder 1
volatile long encoder1Pos = 0;    // encoder 2

double left_kp = 0.0 , left_ki = 0.0 , left_kd = 0.0;             
...
void loop() {
  //Atras
    if(demandx<-0.1){
      //rightPID.SetTunings(3.5,0.08,0.0005);
      //leftPID.SetTunings(4.2,0.1,0.0005);
      rightPID.SetTunings(4.5,0.0,0.0);
      leftPID.SetTunings(4.0,0.0,0.0);
      }
    //Adelante
     if(demandx>0.1){
      //rightPID.SetTunings(3.0,0.1,0.005);
      //leftPID.SetTunings(4.0,0.08,0.0004);
      rightPID.SetTunings(3.0,0.0,0.0);
      leftPID.SetTunings(4.1,0.0,0.0);
      }
     if(demandx==0.0 && demandz==0.0){
      rightPID.SetTunings(0.0,0.0,0.0);
      leftPID.SetTunings(0.0,0.0,0.0);
      }
     if(demandz>0.1){
      rightPID.SetTunings(3.0,0.0,0.0);
      leftPID.SetTunings(3.0,0.0,0.0);
      }
      if(demandz<-0.1){
      rightPID.SetTunings(3.0,0.0,0.0);
      leftPID.SetTunings(3.0,0.0,0.0);
  ...
  if (currentMillis - prevMillis >= LOOPTIME){
    ...
    demand_speed_left = demandx - (demandz*0.12);
    demand_speed_right = demandx + (demandz*0.12);
